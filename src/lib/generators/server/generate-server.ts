  ...
  return {
    statements: [
      ...
      },
      {
        ...
        namedImports: [{kind: StructureKind.ImportSpecifier, name: 'MediaObjectRoute'}]
      },
      {
        kind: StructureKind.ImportDeclaration,
        isTypeOnly: false,
      ...
      },
      {
        ...
        namedImports: [{kind: StructureKind.ImportSpecifier, name: 'BookRoute'}]
      },
      ...importDeclarations,
      {
        kind: StructureKind.ImportDeclaration,
      ...
      },
      {
        ...
        methods: [{
          ...
          {
            name: 'routes',
            statements: ['this.express.use(\'/\', new RootRoute().router);', 'this.express.use(\'/api/docs\', swaggerUI.serve, swaggerUI.setup(yaml.load(\'./swagger.yaml\'), {}));', 'if (process.env.UPLOAD_PROVIDER === \'local\') {\r\n  this.express.use(\'/uploads\', express.static(process.env.UPLOAD_PROVIDER_FOLDER));\r\n}', 'this.express.use(\'/api/auth\', new AuthRoute().router);', 'this.express.use(\'/api/users\', new UserRoute().router);', 'this.express.use(\'/api/resource-permissions\', new ResourcePermissionRoute().router);', 'this.express.use(\'/api/roles\', new RoleRoute().router);', 'this.express.use(\'/api/media-objects\', new MediaObjectRoute().router);', 'this.express.use(\'/api/endpoints\', new EndpointRoute().router);', 'this.express.use(\'/api/books\', new BookRoute().router);', ...registerRoutes],
            docs: [{kind: StructureKind.JSDoc, description: '\r\nRegister routes',}],
            isGenerator: false,
            isAsync: false,
            isStatic: false,
